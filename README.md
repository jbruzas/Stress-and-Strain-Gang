...
The interpretation of the results is that the arm is functional, but has several problems that need to be overcome. There needs to be some way to increase the friction between the grippers and an object and there needs to be some way to stop the intersection between the claw and the upper arm. The fix for the friction issue to to add a rough material to the grippers. The obvious choice for this material is rubber. Rubber would have been added to the grippers after the tests if there was enough time, but there was not enough time to add rubber. The intersection between the claw and upper arm was caused by a scaling issue in SOLIDWORKS. The full scale model made in Phase 2 did not have this intersection issue, but scaling that mode down and 3D printing somehow caused an intersection issue. The specifics behind this scaling issue and how it should be fixed will be talked about in the faulures, mistakes, and surprises section.  
